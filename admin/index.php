...
<body>
 ...
 <header>
        ...
        <nav class="navbar">
            <a href="index.php" class="active">อนุมัติผู้ใช้งานระบบ</a>
...
<table class="table table-hover form-contorl primary" >
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><br>
            <?php if($row['status'] == 0){ ?>
                <a  href = "../admin_php/u_appove.php?id=<?php echo $row['ID'] ?>" class="btn btn-lg btn-success">อนุมัติ</button>
            <?php }else{ ?>
                <a  href = "../admin_php/u_appove.php?id=<?php echo $row['ID'] ?>" class="btn btn-lg btn-danger" >ยกเลิก</button>
            </td>
        </tr>
            <?php } ?>
        <?php } ?>
